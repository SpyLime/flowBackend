// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

/*
 * Flow Learning - OpenAPI 3.1
 *
 * api for flow learning
 *
 * API version: 1.0.0
 * Contact: floTeam@gmail.com
 */

package openapi




type GetMapById200Response struct {

	Nodes []GetMapById200ResponseNodesInner `json:"nodes,omitempty"`

	Edges []GetMapById200ResponseEdgesInner `json:"edges,omitempty"`
}

// AssertGetMapById200ResponseRequired checks if the required fields are not zero-ed
func AssertGetMapById200ResponseRequired(obj GetMapById200Response) error {
	for _, el := range obj.Nodes {
		if err := AssertGetMapById200ResponseNodesInnerRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Edges {
		if err := AssertGetMapById200ResponseEdgesInnerRequired(el); err != nil {
			return err
		}
	}
	return nil
}

// AssertGetMapById200ResponseConstraints checks if the values respects the defined constraints
func AssertGetMapById200ResponseConstraints(obj GetMapById200Response) error {
	for _, el := range obj.Nodes {
		if err := AssertGetMapById200ResponseNodesInnerConstraints(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Edges {
		if err := AssertGetMapById200ResponseEdgesInnerConstraints(el); err != nil {
			return err
		}
	}
	return nil
}
